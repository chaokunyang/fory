...
- For Rust types owned by another crate, name the feature "external-type serialization." Call a
  schema-derived implementation an "external structural serializer," a customized implementation a
  "manual serializer," and Fory's recursive Option/wrapper/list/set/map/tuple/array implementations
  "carrier serializers." The internal type-level role may be called a "serializer provider," but do
  not introduce a separate public marker, derive, attribute, registration API, example suffix, or
  alternate feature concept for the same ownership role.
