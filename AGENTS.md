...
- Run old/new benchmark comparisons for only one language at a time. Do not batch multiple language comparisons into one long benchmark session, because longer sessions increase machine-state drift and variance.
- Treat a same-benchmark gap greater than 1% as a likely hot-path regression until repeated focused measurements or profiling prove it is noise.
