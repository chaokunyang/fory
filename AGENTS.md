...
- Run and close old/new benchmark comparisons for exactly one language at a time. If that language has an absolute gap greater than 1%, keep working only on that language until the gap is within 1% before moving to the next language.
- Treat a same-benchmark slowdown greater than 1% as unresolved until the retained median is within 1% of the baseline. If the current branch is faster, verify the generated code, benchmark shape, safety checks, and protocol semantics did not skip required work, then keep the faster implementation. Do not add artificial slowdowns or benchmark-shape changes to force a match.
