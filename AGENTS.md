...
- Treat an absolute same-benchmark gap greater than 1%, faster or slower, as unresolved until the retained median is within 1%. Do not close the task just because the current branch is faster, and do not add artificial slowdowns or benchmark-shape changes to force a match.
