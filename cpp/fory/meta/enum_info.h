...
/// Register an enum inside a class body (friend ADL hook).
/// Usage:
/// ```cpp
/// struct Holder {
///   enum class Kind { A, B };
///   FORY_ENUM_IN_CLASS(Kind, A, B);
/// };
/// ```
#define FORY_ENUM_IN_CLASS(EnumType, ...)                                      \
  struct FORY_ENUM_DESCRIPTOR_NAME(__LINE__) {                                 \
    using Enum = EnumType;                                                     \
    static constexpr bool defined = true;                                      \
    static constexpr std::size_t size = FORY_PP_NARG(__VA_ARGS__);             \
    static inline constexpr std::array<Enum, size> values = {                  \
  ...
  friend constexpr auto ForyEnumInfo(::fory::meta::Identity<EnumType>) {       \
    return FORY_ENUM_DESCRIPTOR_NAME(__LINE__){};                              \
  }                                                                            \
  static_assert(true)
