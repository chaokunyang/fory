...
WriteContext::WriteContext(const Config &config,
    ...
    : buffer_(), write_buffer_(&buffer_), config_(&config),
...
void WriteContext::write_type_meta(const TypeInfo *type_info) {
  ...
  if (!type_info_index_map_active_) {
    if (!has_first_type_info_) {
      ...
      buffer().write_uint8(0); // (index << 1), index=0
      buffer().write_bytes(type_info->type_def.data(),
                           type_info->type_def.size());
      return;
    }
    if (type_info == first_type_info_) {
      buffer().write_uint8(1); // (index << 1) | 1, index=0
      return;
    }
  ...
  if (auto *entry = write_type_info_index_map_.find(key)) {
    ...
    if (marker < 0x80) {
      buffer().write_uint8(static_cast<uint8_t>(marker));
    } else {
      buffer().write_var_uint32(marker);
    }
    return;
  }
  ...
  buffer().write_bytes(type_info->type_def.data(), type_info->type_def.size());
}
...
WriteContext::write_enum_type_info(const std::type_index &type) {
  ...
  if (type_id == static_cast<uint32_t>(TypeId::ENUM)) {
    ...
    buffer().write_var_uint32(type_info->user_type_id);
    ...
    if (config_->compatible) {
      ...
      if (type_info->encoded_namespace && type_info->encoded_type_name) {
        write_encoded_meta_string(buffer(), *type_info->encoded_namespace);
        write_encoded_meta_string(buffer(), *type_info->encoded_type_name);
      } else {
        return Unexpected(
...
WriteContext::write_any_type_info(uint32_t fory_type_id,
  ...
  if (is_internal_type(fory_type_id)) {
    // write type_id
    buffer().write_uint8(static_cast<uint8_t>(fory_type_id));
