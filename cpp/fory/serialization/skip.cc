...
void skip_struct(ReadContext &ctx, const FieldType &field_type) {
  ...
  auto type_info_result = ctx.get_type_info_by_index(meta_index);
  if (FORY_PREDICT_FALSE(!type_info_result.ok())) {
    ctx.set_error(std::move(type_info_result).error());
    return;
  }
...
void skip_ext(ReadContext &ctx, const FieldType &field_type) {
  ...
  if (tid == TypeId::NAMED_EXT) {
    ...
    type_info = type_info_result.value();
  } else {
    // ID-based ext: look up by full type_id
    // The ext fields in TypeMeta store the user type_id (high bits | EXT)
    auto type_info_by_id = ctx.type_resolver().get_type_info_by_id(full_type_id);
    if (FORY_PREDICT_FALSE(!type_info_by_id.ok())) {
      ctx.set_error(std::move(type_info_by_id).error());
      return;
    }
    type_info = type_info_by_id.value();
  }
  ...
  if (!type_info->harness.valid() || !type_info->harness.read_data_fn) {
    ctx.set_error(Error::type_error(
        "Ext harness not found or incomplete for type: " +
        std::to_string(full_type_id)));
    return;
  }
  ...
  auto ptr_res = type_info->harness.read_data_fn(ctx);
  if (FORY_PREDICT_FALSE(!ptr_res.ok())) {
    ctx.set_error(std::move(ptr_res).error());
    return;
  }
  void *ptr = ptr_res.value();
  if (ptr) {
...
void skip_field_value(ReadContext &ctx, const FieldType &field_type,
  ...
  case TypeId::TIMESTAMP: {
    ...
    if (ctx.buffer().reader_index() + kBytes > ctx.buffer().size()) {
      ctx.set_error(Error::buffer_out_of_bound(
