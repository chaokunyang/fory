    ...
    if (ref_mode == RefMode::None) {
      if (!opt.has_value()) {
        ctx.set_error(Error::invalid(
            "std::optional requires ref_mode != RefMode::None to encode null state"));
        return;
      }
...
template <typename T> struct Serializer<std::shared_ptr<T>> {
  ...
  static inline void write(const std::shared_ptr<T> &ptr, WriteContext &ctx,
    ...
    if (ref_mode == RefMode::None) {
      if (!ptr) {
        ctx.set_error(Error::invalid(
            "std::shared_ptr requires ref_mode != RefMode::None to encode null state"));
        return;
      }
      ...
      if constexpr (is_polymorphic) {
        ...
        Serializer<T>::write(*ptr, ctx, inner_requires_ref ? RefMode::NullOnly : RefMode::None, write_type);
      }
      return;
    }
  ...
  static inline std::shared_ptr<T> read(ReadContext &ctx, RefMode ref_mode,
    ...
    if (ref_mode == RefMode::None) {
      if constexpr (is_polymorphic) {
        ...
        constexpr RefMode inner_ref_mode =
            inner_requires_ref ? RefMode::NullOnly : RefMode::None;
        T value = Serializer<T>::read(ctx, inner_ref_mode, read_type);
        if (ctx.has_error()) {
          return nullptr;
        }
    ...
    if constexpr (is_polymorphic) {
      ...
      // Non-polymorphic path
      T value = Serializer<T>::read(ctx, inner_requires_ref ? RefMode::NullOnly : RefMode::None, read_type);
      if (ctx.has_error()) {
        return nullptr;
      }
  ...
  read_with_type_info(ReadContext &ctx, RefMode ref_mode,
    ...
    if (ref_mode == RefMode::None) {
      ...
      if constexpr (is_polymorphic) {
        ...
        // Non-polymorphic path
        T value = Serializer<T>::read_with_type_info(ctx, inner_requires_ref ? RefMode::NullOnly : RefMode::None,
                                                     type_info);
        if (ctx.has_error()) {
          return nullptr;
        }
...
template <typename T> struct Serializer<std::unique_ptr<T>> {
  ...
  static inline void write(const std::unique_ptr<T> &ptr, WriteContext &ctx,
    ...
    if (ref_mode == RefMode::None) {
      if (!ptr) {
        ctx.set_error(Error::invalid(
            "std::unique_ptr requires ref_mode != RefMode::None to encode null state"));
        return;
      }
      // For polymorphic types, serialize the concrete type dynamically
      if constexpr (is_polymorphic) {
