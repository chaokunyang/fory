...
TEST(SchemaEvolutionTest, TaggedBinaryFieldKeepsExplicitIdMatch) {
  ...
  ASSERT_TRUE(
      fory_reduced.register_struct<MatchTaggedBinaryFieldReduced>(TYPE_ID).ok());
...
TEST(SchemaEvolutionTest, TaggedInt8FieldKeepsExplicitIdMatch) {
  ...
  ASSERT_TRUE(fory_full.register_struct<MatchTaggedInt8FieldFull>(TYPE_ID).ok());
  ASSERT_TRUE(
  ...
  auto deser_result = fory_reduced.deserialize<MatchTaggedInt8FieldReduced>(
      ser_result.value());
  ASSERT_TRUE(deser_result.ok()) << deser_result.error().to_string();
