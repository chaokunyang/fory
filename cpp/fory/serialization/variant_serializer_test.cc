...
TEST(VariantSerializerTest, BasicVariant) {
  ...
  // Test with int active
  {
    ...
    auto read_result1 = fory.deserialize<std::variant<int, std::string, double>>(
        result1.value());
    if (!read_result1.has_value()) {
      std::cerr << "Deserialization error: " << read_result1.error().message() << std::endl;
    }
    ASSERT_TRUE(read_result1.has_value());
  ...
  // Test with string active
  {
    ...
    auto read_result2 = fory.deserialize<std::variant<int, std::string, double>>(
        result2.value());
  ...
  // Test with double active
  {
    ...
    auto read_result3 = fory.deserialize<std::variant<int, std::string, double>>(
        result3.value());
...
TEST(VariantSerializerTest, ComplexVariant) {
  ...
  std::variant<std::vector<int>, std::string> v1 = std::vector<int>{1, 2, 3, 4, 5};
  auto result = fory.serialize(v1);
  ASSERT_TRUE(result.has_value());
  
  auto read_result = fory.deserialize<std::variant<std::vector<int>, std::string>>(
      result.value());
...
TEST(VariantSerializerTest, XlangMode) {
  ...
  auto read_result = fory.deserialize<std::variant<int, std::string, bool>>(
      result.value());
  ASSERT_TRUE(read_result.has_value());
...
TEST(VariantSerializerTest, MonostateVariant) {
  ...
  auto read_result = fory.deserialize<std::variant<std::monostate, int, std::string>>(
      result.value());
  ASSERT_TRUE(read_result.has_value());
  ASSERT_EQ(read_result.value().index(), 0);
