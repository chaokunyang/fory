...
mutable third-party targets, including exact private fields or properties and
renamed member mappings. Use a manual serializer for immutable,
constructor-only, factory-only, readonly, init-only, converted, or custom-wire
targets. On .NET 8, a private external member whose declaring type or signature
is generic also requires a manual serializer.
