...
`PersonForyModule` is generated by `build_runner`. The `typeName` value is how peers in other languages identify the same type; keep it stable once your service is in production. Use `.` inside `typeName` to add a namespace prefix.
