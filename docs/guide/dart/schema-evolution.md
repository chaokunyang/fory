...
- Change the registration identity (`id` or `typeName`) of a type after messages are in production.
...
2. The same type registration identity (numeric ID or `typeName`).
