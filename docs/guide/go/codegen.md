...
Code generation is an **experimental** feature in Fory Go. The API and behavior may change in future releases. The standard reflective serialization path remains the stable, recommended approach for most use cases.
:::
...
type ColdPathStruct struct {
    // Not annotated, uses the standard reflective serializer
}
```
