...
| `maxSchemaVersionsPerType`          | Maximum accepted remote metadata versions for one logical type before Fory rejects additional remote metadata on cache misses. Struct types may have multiple schema-evolution versions; compatible named enum/ext/union metadata normally has one version but still counts when it is sent as shared metadata. Pure id-based enum/ext/typed-union values use type id plus user type id and do not carry TypeDef metadata.                                                                                                                    | `10`                                                                 |
...
- `withMaxTypeFields(...)` and `withMaxTypeMetaBytes(...)` bound the field count
  and encoded body size of one received remote metadata body before Fory builds
  read state from it.
- `withMaxSchemaVersionsPerType(...)` and
  `withMaxAverageSchemaVersionsPerType(...)` bound remote metadata cache misses
  without changing registration, dynamic loading, or schema-evolution semantics.
