...
root attempt, even when that operation fails, so its explicit type and serializer mappings stay
fixed. Native modes may still perform documented lazy runtime-type discovery when registration is
not required.
