...
those authorized by the configured policy. That lazy resolution does not add an explicit
registration. Explicit names, IDs, and custom serializers must be configured before the first
root; later registration attempts fail.
