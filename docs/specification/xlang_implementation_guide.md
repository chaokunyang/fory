...
deserialization. Starting either root establishes exactly one authoritative frozen lifecycle fact
for the
...
lifecycle. That fact may be an owner-native flag or the presence of an immutable registry snapshot.
Do not add another lifecycle state, a registration commit or rollback path, or eager whole-registry
