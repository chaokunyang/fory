...
// ============================================================================
// GenericMapSerializer - Type-safe generic map serializer
// ============================================================================

// GenericMapSerializer provides type-safe serialization for maps of type map[K]V.
// It uses static type dispatch instead of runtime type ID lookup for collections.
type GenericMapSerializer[K comparable, V any] struct {
	keySerializer   Serializer
	valueSerializer Serializer
	keyTypeId       TypeId
	valueTypeId     TypeId
...
// NewGenericMapSerializer creates a new generic map serializer for type map[K]V
func NewGenericMapSerializer[K comparable, V any](
	keySerializer Serializer,
	valueSerializer Serializer,
	keyTypeId TypeId,
	valueTypeId TypeId,
) *GenericMapSerializer[K, V] {
	return &GenericMapSerializer[K, V]{
...
func (s *GenericMapSerializer[K, V]) TypeId() TypeId {
	return MAP
}

func (s *GenericMapSerializer[K, V]) NeedToWriteRef() bool {
	return true
}
...
func (s *GenericMapSerializer[K, V]) WriteReflect(ctx *WriteContext, value reflect.Value) error {
	...
	// Write chunk header and entries
	buf.WriteByte_(chunkHeader)
	...
	for iter.Next() {
		...
		// Write key
		if trackKeyRef {
			refWritten, err := ctx.RefResolver().WriteRefOrNull(buf, k)
			if err != nil {
				return err
			}
			if !refWritten {
		...
		// Write value
		if trackValueRef {
			refWritten, err := ctx.RefResolver().WriteRefOrNull(buf, v)
			if err != nil {
				return err
			}
			if !refWritten {
...
func (s *GenericMapSerializer[K, V]) ReadReflect(ctx *ReadContext, type_ reflect.Type, value reflect.Value) error {
	...
	// Create map
	m := reflect.MakeMapWithSize(type_, size)
	value.Set(m)
	...
	for i := 0; i < size; i++ {
		...
		if trackKeyRef {
			...
			if int8(refID) < NotNullValueFlag {
				...
				if refID >= 0 {
					ctx.RefResolver().SetReadObject(refID, k)
				}
			}
		} else {
			if err := s.keySerializer.ReadReflect(ctx, keyType, k); err != nil {
		...
		if trackValueRef {
			...
			if int8(refID) < NotNullValueFlag {
				...
				if refID >= 0 {
					ctx.RefResolver().SetReadObject(refID, v)
				}
			}
		} else {
			if err := s.valueSerializer.ReadReflect(ctx, valueType, v); err != nil {
