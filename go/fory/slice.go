...
func (s *sliceSerializer) writeDataWithGenerics(ctx *WriteContext, value reflect.Value, hasGenerics bool) {
	...
	for i := 0; i < length; i++ {
		...
		if trackRefs {
			...
			s.elemSerializer.Write(ctx, elemRefMode, false, declaredGenerics, elem)
		} else if hasNull {
			// When hasNull is set but trackRefs is not, write NotNullValueFlag before data
			buf.WriteInt8(NotNullValueFlag)
			writeSerializerData(ctx, s.elemSerializer, declaredGenerics, elem)
		} else {
...
func (s *sliceSerializer) ReadData(ctx *ReadContext, value reflect.Value) {
	...
	for i := 0; i < length; i++ {
		...
		if trackRefs {
			...
			s.elemSerializer.Read(ctx, elemRefMode, false, declaredGenerics, elem)
		} else if hasNull {
			...
			readSerializerData(ctx, s.elemSerializer, declaredGenerics, elem)
		} else {
