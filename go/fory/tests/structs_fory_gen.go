...
// source: structs.go
// generated at: 2026-07-01T02:06:03+08:00
...
func (g *DynamicSliceDemo_ForyGenSerializer) ReadTyped(ctx *fory.ReadContext, v *DynamicSliceDemo) error {
	...
	// Dynamic slice []any handling - manual deserialization
	{
		...
		if isXlang {
			...
			sliceLen := ctx.ReadCollectionLength()
			if ctx.HasError() {
				return ctx.TakeError()
			}
			{
				graphCount := sliceLen
				graphElemBytes := int64(int64(unsafe.Sizeof(*new(any))))
				if graphCount < 0 {
					ctx.SetError(fory.DeserializationErrorf("negative graph element count: %d", graphCount))
					return ctx.TakeError()
				}
				if graphElemBytes < 0 {
					ctx.SetError(fory.DeserializationErrorf("negative graph element size: %d", graphElemBytes))
					return ctx.TakeError()
				}
				if graphCount != 0 && graphElemBytes != 0 && int64(graphCount) > fory.MaxInt64/graphElemBytes {
					ctx.SetError(fory.DeserializationErrorf("graph memory estimate overflows: length=%d elementBytes=%d", graphCount, graphElemBytes))
					return ctx.TakeError()
				}
				if !ctx.ReserveGraphMemory(int64(graphCount) * graphElemBytes) {
					return ctx.TakeError()
				}
			}
			if sliceLen == 0 {
				...
				if !buf.CheckReadable(sliceLen, err) {
					return ctx.TakeError()
				}
...
func (g *DynamicSliceDemo_ForyGenSerializer) ReadData(ctx *fory.ReadContext, value reflect.Value) {
	...
	if value.Kind() == reflect.Ptr {
		if value.IsNil() {
			graphBytes := int64(unsafe.Sizeof(DynamicSliceDemo{}))
			if graphBytes == 0 {
				graphBytes = 1
			}
...
func (g *MapDemo_ForyGenSerializer) ReadTyped(ctx *fory.ReadContext, v *MapDemo) error {
	...
	// Field: IntMap (map[int]int)
	{
		...
		if isXlang {
			...
			mapLen := ctx.ReadCollectionLength()
			if ctx.HasError() {
				return ctx.TakeError()
			}
			{
				graphCount := mapLen
				graphElemBytes := int64(int64(unsafe.Sizeof(*new(int))) + int64(unsafe.Sizeof(*new(int))))
				if graphCount < 0 {
					ctx.SetError(fory.DeserializationErrorf("negative graph element count: %d", graphCount))
					return ctx.TakeError()
				}
			...
			if mapLen == 0 {
				...
				if !buf.CheckReadable(mapLen, err) {
					return ctx.TakeError()
				}
				...
				for mapSize > 0 {
					...
					if chunkSize == 0 || chunkSize > mapSize {
						ctx.SetError(fory.DeserializationErrorf("invalid map chunk size %d for remaining length %d", chunkSize, mapSize))
						return ctx.TakeError()
					}
	...
	// Field: MixedMap (map[string]int)
	{
		...
		if isXlang {
			...
			{
				graphCount := mapLen
				graphElemBytes := int64(int64(unsafe.Sizeof(*new(string))) + int64(unsafe.Sizeof(*new(int))))
				if graphCount < 0 {
					ctx.SetError(fory.DeserializationErrorf("negative graph element count: %d", graphCount))
					return ctx.TakeError()
				}
	...
	// Field: StringMap (map[string]string)
	{
		...
		if isXlang {
			...
			{
				graphCount := mapLen
				graphElemBytes := int64(int64(unsafe.Sizeof(*new(string))) + int64(unsafe.Sizeof(*new(string))))
				if graphCount < 0 {
					ctx.SetError(fory.DeserializationErrorf("negative graph element count: %d", graphCount))
					return ctx.TakeError()
				}
...
func (g *MapDemo_ForyGenSerializer) ReadData(ctx *fory.ReadContext, value reflect.Value) {
	...
	if value.Kind() == reflect.Ptr {
		if value.IsNil() {
			graphBytes := int64(unsafe.Sizeof(MapDemo{}))
			if graphBytes == 0 {
				graphBytes = 1
			}
			if !ctx.ReserveGraphMemory(graphBytes) {
...
func (g *SliceDemo_ForyGenSerializer) ReadTyped(ctx *fory.ReadContext, v *SliceDemo) error {
	...
	// Field: BoolSlice ([]bool)
	{
		...
		if isXlang {
			...
			{
				graphCount := sliceLen
				graphElemBytes := int64(int64(unsafe.Sizeof(*new(bool))))
				if graphCount < 0 {
					ctx.SetError(fory.DeserializationErrorf("negative graph element count: %d", graphCount))
					return ctx.TakeError()
				}
			...
			if sliceLen == 0 {
				...
				if hasDeclType {
					...
					for i := 0; i < sliceLen; i++ {
						v.BoolSlice[i] = buf.ReadBool(err)
					}
				} else {
	...
	// Field: FloatSlice ([]float64)
	{
		...
		if isXlang {
			...
			{
				graphCount := sliceLen
				graphElemBytes := int64(int64(unsafe.Sizeof(*new(float64))))
				if graphCount < 0 {
					ctx.SetError(fory.DeserializationErrorf("negative graph element count: %d", graphCount))
					return ctx.TakeError()
				}
			...
			if sliceLen == 0 {
				...
				if hasDeclType {
					// Elements are written directly without type IDs
					for i := 0; i < sliceLen; i++ {
						v.FloatSlice[i] = buf.ReadFloat64(err)
					}
				} else {
					// Need to read type ID once if CollectionIsSameType is set
					if (collectFlag & 8) != 0 {
	...
	// Field: IntSlice ([]int32)
	{
		...
		if isXlang {
			...
			{
				graphCount := sliceLen
				graphElemBytes := int64(int64(unsafe.Sizeof(*new(int32))))
				if graphCount < 0 {
					ctx.SetError(fory.DeserializationErrorf("negative graph element count: %d", graphCount))
					return ctx.TakeError()
				}
			...
			if sliceLen == 0 {
				...
				if hasDeclType {
					// Elements are written directly without type IDs
					for i := 0; i < sliceLen; i++ {
						v.IntSlice[i] = buf.ReadVarint32(err)
					}
				} else {
					// Need to read type ID once if CollectionIsSameType is set
					if (collectFlag & 8) != 0 {
	...
	// Field: StringSlice ([]string)
	{
		...
		if isXlang {
			...
			{
				graphCount := sliceLen
				graphElemBytes := int64(int64(unsafe.Sizeof(*new(string))))
				if graphCount < 0 {
					ctx.SetError(fory.DeserializationErrorf("negative graph element count: %d", graphCount))
					return ctx.TakeError()
				}
...
func (g *SliceDemo_ForyGenSerializer) ReadData(ctx *fory.ReadContext, value reflect.Value) {
	...
	if value.Kind() == reflect.Ptr {
		if value.IsNil() {
			graphBytes := int64(unsafe.Sizeof(SliceDemo{}))
			if graphBytes == 0 {
				graphBytes = 1
			}
			if !ctx.ReserveGraphMemory(graphBytes) {
...
func (g *ValidationDemo_ForyGenSerializer) ReadData(ctx *fory.ReadContext, value reflect.Value) {
	...
	if value.Kind() == reflect.Ptr {
		if value.IsNil() {
			graphBytes := int64(unsafe.Sizeof(ValidationDemo{}))
			if graphBytes == 0 {
				graphBytes = 1
			}
			if !ctx.ReserveGraphMemory(graphBytes) {
