...
// source: structs.go
// generated at: 2026-06-30T22:09:02+08:00
...
func (g *DynamicSliceDemo_ForyGenSerializer) ReadTyped(ctx *fory.ReadContext, v *DynamicSliceDemo) error {
	...
	// Dynamic slice []any handling - manual deserialization
	{
		...
		if isXlang {
			...
			if !ctx.ReserveCountedGraphMemory(sliceLen, int64(unsafe.Sizeof(*new(any)))) {
				return ctx.TakeError()
			}
			if sliceLen == 0 {
...
func (g *MapDemo_ForyGenSerializer) ReadTyped(ctx *fory.ReadContext, v *MapDemo) error {
	...
	// Field: IntMap (map[int]int)
	{
		...
		if isXlang {
			...
			if !ctx.ReserveCountedGraphMemory(mapLen, int64(unsafe.Sizeof(*new(int)))+int64(unsafe.Sizeof(*new(int)))) {
				return ctx.TakeError()
			}
			if mapLen == 0 {
	...
	// Field: MixedMap (map[string]int)
	{
		...
		if isXlang {
			...
			if !ctx.ReserveCountedGraphMemory(mapLen, int64(unsafe.Sizeof(*new(string)))+int64(unsafe.Sizeof(*new(int)))) {
				return ctx.TakeError()
			}
			if mapLen == 0 {
	...
	// Field: StringMap (map[string]string)
	{
		...
		if isXlang {
			...
			if !ctx.ReserveCountedGraphMemory(mapLen, int64(unsafe.Sizeof(*new(string)))+int64(unsafe.Sizeof(*new(string)))) {
				return ctx.TakeError()
			}
			if mapLen == 0 {
...
func (g *SliceDemo_ForyGenSerializer) ReadTyped(ctx *fory.ReadContext, v *SliceDemo) error {
	...
	// Field: BoolSlice ([]bool)
	{
		...
		if isXlang {
			...
			if !ctx.ReserveCountedGraphMemory(sliceLen, int64(unsafe.Sizeof(*new(bool)))) {
				return ctx.TakeError()
			}
			if sliceLen == 0 {
	...
	// Field: FloatSlice ([]float64)
	{
		...
		if isXlang {
			...
			if !ctx.ReserveCountedGraphMemory(sliceLen, int64(unsafe.Sizeof(*new(float64)))) {
				return ctx.TakeError()
			}
			if sliceLen == 0 {
	...
	// Field: IntSlice ([]int32)
	{
		...
		if isXlang {
			...
			if !ctx.ReserveCountedGraphMemory(sliceLen, int64(unsafe.Sizeof(*new(int32)))) {
				return ctx.TakeError()
			}
			if sliceLen == 0 {
	...
	// Field: StringSlice ([]string)
	{
		...
		if isXlang {
			...
			if !ctx.ReserveCountedGraphMemory(sliceLen, int64(unsafe.Sizeof(*new(string)))) {
				return ctx.TakeError()
			}
			if sliceLen == 0 {
