...
func (s dateSerializer) Write(ctx *WriteContext, value any) error {
	date := value.(Date)
...
func (s dateSerializer) Read(ctx *ReadContext) (any, error) {
...
func (s dateSerializer) WriteReflect(ctx *WriteContext, value reflect.Value) error {
...
func (s dateSerializer) ReadReflect(ctx *ReadContext, type_ reflect.Type, value reflect.Value) error {
	diff := time.Duration(ctx.buffer.ReadInt32()) * 24 * time.Hour
	date := time.Date(1970, 1, 1, 0, 0, 0, 0, time.Local).Add(diff)
...
func (s timeSerializer) Write(ctx *WriteContext, value any) error {
	ctx.buffer.WriteInt64(GetUnixMicro(value.(time.Time)))
	return nil
}

func (s timeSerializer) Read(ctx *ReadContext) (any, error) {
	return CreateTimeFromUnixMicro(ctx.buffer.ReadInt64()), nil
}

func (s timeSerializer) WriteReflect(ctx *WriteContext, value reflect.Value) error {
	ctx.buffer.WriteInt64(GetUnixMicro(value.Interface().(time.Time)))
	return nil
}

func (s timeSerializer) ReadReflect(ctx *ReadContext, type_ reflect.Type, value reflect.Value) error {
