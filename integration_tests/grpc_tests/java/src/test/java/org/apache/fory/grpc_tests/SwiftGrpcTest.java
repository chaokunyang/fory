...
public class SwiftGrpcTest extends GrpcTestBase {

  @Test
  public void testJavaServerSwiftClient() throws Exception {
    Server server = startJavaAllSchemasServer();
    try {
      runPeer(
          "swift-grpc-client",
          swiftCommand("client", "--target", "127.0.0.1:" + server.getPort()));
    } finally {
      server.shutdownNow();
  ...
  @Test
  public void testSwiftServerJavaClient() throws Exception {
    exercisePeerServer(
        "swift-grpc", "Swift", "fory-grpc-swift-", swiftCommand("server"), this::exerciseAllSchemas);
  }
