...
public class CodecUtils {
  ...
  private static Class<?> codecNeighbor(Class<?> beanClass, ClassLoader beanClassClassLoader) {
    // Hidden generated serializers are only a JDK25+ path for source-non-public bean classes.
    // Source-public beans keep the normal generated class so split helpers remain private; JDK25
    // VarHandle field access is independent of hidden class definition.
    if (AndroidSupport.IS_ANDROID
        || JdkVersion.MAJOR_VERSION < 25
        || beanClass.getClassLoader() == null
        || CodeGenerator.sourcePublicAccessible(beanClass)) {
      return null;
    }
