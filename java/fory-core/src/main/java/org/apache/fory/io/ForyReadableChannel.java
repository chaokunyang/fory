...
public class ForyReadableChannel implements ForyStreamReader, ReadableByteChannel {
  ...
  public void compactBuffer() {
    ...
    // Retaining a smaller consumed prefix is cheaper than copying a larger unread suffix. Once the
    // prefix reaches the suffix size, compaction keeps both buffer growth and byte movement
    // amortized across roots.
    if (readerIndex < unreadBytes) {
      return;
    }
    ByteBuffer byteBuf = byteBuffer;
