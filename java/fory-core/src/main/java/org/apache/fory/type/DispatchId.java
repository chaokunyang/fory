...
public class DispatchId {
  ...
  public static int getDispatchId(Fory fory, Descriptor d) {
    if (fory.isCrossLanguage()) {
      ...
      return xlangTypeIdToDispatchId(typeId, d.getTypeRef().isPrimitive());
    } else {
      int typeId = Types.getDescriptorTypeId(fory, d);
      return nativeIdToDispatchId(typeId, d.getTypeRef().isPrimitive());
    }
  }
  ...
  private static int nativeIdToDispatchId(int nativeId, boolean isPrimitive) {
    ...
    if (nativeId >= ClassResolver.PRIMITIVE_VOID_ID && nativeId <= ClassResolver.PRIMITIVE_FLOAT64_ID) {
      throw new IllegalArgumentException("Should use `Types.BOOL~Types.FLOAT64` with nullable meta instead");
    }
    return xlangTypeIdToDispatchId(nativeId, isPrimitive);
  }
