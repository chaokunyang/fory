...
public class ObjectCodecBuilder extends BaseObjectCodecBuilder {
  ...
  private int[] buildConstructorFieldIndexes(
    ...
    for (int i = 0; i < names.length; i++) {
      ...
      indexes[i] =
          constructorFieldIndex(descriptors, declaringClass, names[i], allowMissing);
    }
    return indexes;
  }
  ...
  private List<Expression> serializePrimitivesUnCompressed(
    ...
    Expression writerIndex =
        new Invoke(buffer, "writerIndex", "writerIndex", PRIMITIVE_INT_TYPE);
    ...
    for (List<Descriptor> group : primitiveGroups) {
      ...
      for (Descriptor descriptor : group) {
        ...
        if (dispatchId == DispatchId.BOOL) {
          groupExpressions.add(bufferPutBoolean(buffer, getBufferIndex(writerIndex, acc), fieldValue));
          acc += 1;
          ...
          groupExpressions.add(
              bufferPutByte(
                  buffer, getBufferIndex(writerIndex, acc), primitiveByteValue(fieldValue, descriptor)));
          acc += 1;
          ...
          groupExpressions.add(bufferPutInt16(buffer, getBufferIndex(writerIndex, acc), fieldValue));
          acc += 2;
          ...
          groupExpressions.add(bufferPutInt32(buffer, getBufferIndex(writerIndex, acc), fieldValue));
          acc += 4;
        } else if (dispatchId == DispatchId.UINT32) {
          groupExpressions.add(
              bufferPutInt32(
                  buffer, getBufferIndex(writerIndex, acc), primitiveIntValue(fieldValue, descriptor)));
          acc += 4;
        } else if (dispatchId == DispatchId.INT64 || dispatchId == DispatchId.UINT64) {
          groupExpressions.add(bufferPutInt64(buffer, getBufferIndex(writerIndex, acc), fieldValue));
          acc += 8;
        } else if (dispatchId == DispatchId.FLOAT32) {
          groupExpressions.add(bufferPutFloat32(buffer, getBufferIndex(writerIndex, acc), fieldValue));
          acc += 4;
        } else if (dispatchId == DispatchId.FLOAT64) {
          groupExpressions.add(bufferPutFloat64(buffer, getBufferIndex(writerIndex, acc), fieldValue));
          acc += 8;
        } else {
  ...
  private Expression deserializeToBean(Expression bean, Reference buffer, Descriptor descriptor) {
    TypeRef<?> castTypeRef =
        hasCompatibleCollectionArrayRead(descriptor)
            ...
            : descriptor.getTypeRef();
    return deserializeField(
        buffer, descriptor, expr -> setFieldValue(bean, descriptor, tryInlineCast(expr, castTypeRef)));
  }
  ...
  protected Expression createConstructorObject(FieldsArray fieldValues) {
    ...
    ObjectCreator<?> objectCreator = ObjectCreators.getObjectCreator(beanClass);
    if (objectCreator.isOnlyPublicConstructor()
