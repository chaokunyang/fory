...
public class SerializedLambdaSerializer extends Serializer {
  ...
  static {
    MethodHandle readResolveHandle = null;
    if (AndroidSupport.IS_ANDROID) {
      // Lambda serialization is unsupported on Android.
    } else {
      try {
        ...
        try {
          ...
          // JDK25 rejects privateLookupIn for java.lang.invoke.SerializedLambda itself. With
          // java.lang.invoke opened, reflective access still exposes the JDK readResolve method
          // without using Unsafe.
          readResolveMethod.setAccessible(true);
          readResolveHandle = java.lang.invoke.MethodHandles.lookup().unreflect(readResolveMethod);
        }
      } catch (IllegalAccessException | RuntimeException e) {
        // Keep serializer registration available; readResolve reports the missing open if used.
      }
    }
    READ_RESOLVE_HANDLE = readResolveHandle;
  }
  ...
  static Object readResolve(Object replacement) {
    throwIfAndroid();
    if (READ_RESOLVE_HANDLE == null) {
      throw new ForyException(
          "SerializedLambda.readResolve is inaccessible. On JDK25+, deserialize lambdas only "
              + "with --add-opens=java.base/java.lang.invoke="
