...
public class PrimitiveSerializersTest extends ForyTestBase {
  ...
  public void testPrimitiveStruct(boolean compressNumber, boolean codegen) {
    ...
    if (compressNumber) {
      ...
      serDeCheck(
          builder.withNumberCompressed(true).withLongCompressed(LongEncoding.VARINT64).build(), struct);
      serDeCheck(
          builder.withNumberCompressed(true).withLongCompressed(LongEncoding.TAGGED_INT64).build(), struct);
    } else {
      Fory fory =
          Fory.builder()
              .withLanguage(Language.JAVA)
