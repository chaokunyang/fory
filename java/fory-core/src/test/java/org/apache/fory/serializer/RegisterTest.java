...
public class RegisterTest extends ForyTestBase {
  ...
  @Test
  public void testStreamRootFreezesBeforeBuffer() {
    Fory reader = Fory.builder().requireClassRegistration(false).build();
    byte[] bytes = Fory.builder().requireClassRegistration(false).build().serialize("value");
    AtomicBoolean registrationRejected = new AtomicBoolean();
    ForyInputStream inputStream =
        new ForyInputStream(new ByteArrayInputStream(bytes)) {
          @Override
          public MemoryBuffer getBuffer() {
            try {
              reader.register(MyExt.class);
