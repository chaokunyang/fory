 ...
 * companion provides direct member access and creator invocation on the JVM and Android. GraalVM
 * Native Image discovers this annotation directly and does not use annotation-processor output. A
 * concrete subtype listed only by a class-literal {@link JsonSubTypes} entry receives retention
 * metadata but needs its own direct {@code JsonType} annotation to receive a companion on the JVM
 * or Android. Outside Native Image, a directly annotated model that reaches the default object
 * codec fails during codec creation when its generated companion is missing.
 *
