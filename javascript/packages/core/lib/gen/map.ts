...
class MapChunkWriter {
  ...
  constructor(private writeContext: WriteContext, private keySerializer?: Serializer | null, private valueSerializer?: Serializer | null) {
  ...
  private getHead(keyInfo: ElementInfo, valueInfo: ElementInfo) {
    ...
    if (!valueInfo.isNull && valueInfo.declared) {
    ...
    if (!keyInfo.isNull && keyInfo.declared) {
      flag |= MapFlags.DECL_ELEMENT_TYPE;
    }
    return flag;
  }
...
class MapAnySerializer {
  ...
  private canDeclareSerializer(serializer: Serializer | null | undefined) {
    if (!serializer) {
      return false;
    }
    return this.writeContext.typeResolver.isMonomorphic(serializer.getTypeInfo());
  }
  ...
  write(value: Map<any, any>) {
    const mapChunkWriter = new MapChunkWriter(this.writeContext, this.keySerializer, this.valueSerializer);
    this.writeContext.writer.writeVarUint32Small7(value.size);
    for (const [k, v] of value.entries()) {
      const keySerializer = this.keySerializer !== null ? this.keySerializer : this.writeContext.typeResolver.getSerializerByData(k);
      const valueSerializer = this.valueSerializer !== null ? this.valueSerializer : this.writeContext.typeResolver.getSerializerByData(v);
      ...
      const header = mapChunkWriter.next(
        new ElementInfo(
          ...
          this.keySerializer !== null && this.canDeclareSerializer(keySerializer),
        ),
        new ElementInfo(
          valueSerializer || null,
          v == null,
          valueSerializer?.needToWriteRef() || false,
          this.valueSerializer !== null && this.canDeclareSerializer(valueSerializer),
        )
      );
...
export class MapSerializerGenerator extends BaseSerializerGenerator {
  ...
  write(accessor: string): string {
    ...
    return `new (${anySerializer})(${this.builder.getWriteContextName()}, ${this.builder.getReadContextName()}, ${this.typeInfo.options!.key!.typeId !== TypeId.UNKNOWN ? innerSerializer(this.typeInfo.options!.key!) : null
  ...
  private readSpecificType(accessor: (expr: string) => string, refState: string) {
    ...
    const readDetectedSerializer = (serializerExpr: string, target: string, refFlag: string) => {
      return `
        ${readContextName}.incReadDepth();
        ${target} = ${serializerExpr}.read(${refFlag});
        ${readContextName}.decReadDepth();
      `;
    };
    const readKeyOrDetected = (refFlag: string) => {
      return `
        if (${keySerializer}) {
          ${readDetectedSerializer(keySerializer, "key", refFlag)}
        } else {
          ${readKey(x => `key = ${x}`, refFlag)}
        }
      `;
    };
    const readValueOrDetected = (refFlag: string) => {
      return `
        if (${valueSerializer}) {
          ${readDetectedSerializer(valueSerializer, "value", refFlag)}
        } else {
          ${readValue(x => `value = ${x}`, refFlag)}
        }
      `;
    };
    ...
    return `
      ...
      while (${count} > 0) {
        ...
        if (!keyIncludeNone && !(keyHeader & ${MapFlags.DECL_ELEMENT_TYPE})) {
          ${keySerializer} = ${anyHelper}.detectSerializer(${readContextName});
        }
        if (!valueIncludeNone && !(valueHeader & ${MapFlags.DECL_ELEMENT_TYPE})) {
  ...
  read(accessor: (expr: string) => string, refState: string): string {
    ...
    return accessor(`new (${anySerializer})(${this.builder.getWriteContextName()}, ${this.builder.getReadContextName()}, ${this.typeInfo.options!.key!.typeId! !== TypeId.UNKNOWN ? innerSerializer(this.typeInfo.options!.key!) : null
      }, ${this.typeInfo.options!.value!.typeId !== TypeId.UNKNOWN ? innerSerializer(this.typeInfo.options!.value!) : null
      }).read(${refState})`);
  }
