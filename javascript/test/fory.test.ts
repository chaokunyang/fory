...
describe('fory', () => {
    ...
    test('gets root codec for plain object struct roots', () => {
        const fory = new Fory({ compatible: true })
        fory.register(Type.struct(100, {
            name: Type.string().setId(1),
        }))

        const codec = fory.getRootCodec<{ name: string }>({
            kind: 'struct',
            typeId: 100,
        })
        ...
        expect(() => fory.serialize(input)).toThrow(
            'Failed to detect the Fory type from JavaScript type: object',
        )
    })

    test('gets root codec for named struct roots', () => {
        const fory = new Fory({ compatible: true })
    ...
    test('gets exact root codec for union roots', () => {
        ...
        const codec = fory.getRootCodec<{ case: number; value: string | number }>({
            kind: 'union',
            typeId: 200,
        })
    ...
    test('gets exact root codec for named union roots', () => {
        const fory = new Fory({ compatible: true })
        fory.register(Type.union({
            namespace: 'example',
            typeName: 'Choice',
        }, {
            1: Type.string(),
