...
private object ReentrantRegistration {
  ...
  var canonicalSerializer: Serializer<*>? = null
  var candidate: Serializer<*>? = null
  var constructions: Int = 0

  fun reset() {
    fory = null
    canonical = null
    canonicalSerializer = null
    candidate = null
    constructions = 0
  }
}
...
class ReentrantStruct_ForySerializer(resolver: TypeResolver, cls: Class<*>) :
  StaticGeneratedStructSerializer<ReentrantStruct>(resolver, cls as Class<ReentrantStruct>) {
  init {
    ReentrantRegistration.constructions++
    ReentrantRegistration.canonical = resolver.getTypeInfo(cls, false)
    ReentrantRegistration.canonicalSerializer =
      ReentrantRegistration.canonical?.getSerializer<ReentrantStruct>()
    ReentrantRegistration.candidate = this
  ...
  override fun readCompatible(readContext: ReadContext): ReentrantStruct = ReentrantStruct()

  override fun copy(copyContext: CopyContext, value: ReentrantStruct): ReentrantStruct =
    ReentrantStruct()

  override fun getGeneratedDescriptors(): List<Descriptor> = emptyList()
}
...
class BuiltinClassSerializerTests {
  ...
  fun testCombinedFreezeRecheck() {
    ...
    try {
      ...
      val resolver = fory.typeResolver
      val canonical = checkNotNull(ReentrantRegistration.canonical)
      val candidate = checkNotNull(ReentrantRegistration.candidate)
      Assert.assertEquals(ReentrantRegistration.constructions, 1)
      Assert.assertTrue(candidate is StaticGeneratedStructSerializer<*>)
      Assert.assertTrue(resolver.isRegistered(ReentrantStruct::class.java))
      Assert.assertSame(resolver.getTypeInfo(ReentrantStruct::class.java, false), canonical)
      Assert.assertSame(
        canonical.getSerializer<ReentrantStruct>(),
        ReentrantRegistration.canonicalSerializer,
      )
      Assert.assertNotSame(canonical.getSerializer<ReentrantStruct>(), candidate)
    } finally {
      ReentrantRegistration.reset()
    }
  }
  ...
  fun testMissingGeneratedType() {
    ...
    assertThrows(IllegalArgumentException::class.java) {
      KotlinSerializers.registerSerializer(fory, ReentrantStruct::class.java)
    }
    ...
    Assert.assertNull(fory.typeResolver.getTypeInfo(ReentrantStruct::class.java, false))
  }
  ...
  fun testSharedDefaultValueSupport() {
    ...
    Assert.assertEquals(support.getDefaultValue(ClassWithDefaults::class.java, "x"), 1)

    ForyKotlin.builder().withXlang(false).requireClassRegistration(true).build()
    Assert.assertSame(DefaultValueUtils.getKotlinDefaultValueSupport(), support)
    Assert.assertEquals(support.getDefaultValue(ClassWithDefaults::class.java, "x"), 1)
  }
