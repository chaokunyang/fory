...
def test_cross_language_serializer(data_file_path):
    with open(data_file_path, "rb") as f:
        data_bytes = f.read()
        buffer = pyfory.Buffer(data_bytes)
        fory = pyfory.Fory(language=pyfory.Language.XLANG, ref=True)
        objects = []
...
def test_serialize_simple_struct(data_file_path):
    ...
    fory = pyfory.Fory(language=pyfory.Language.XLANG, ref=True, compatible=compatible)
    fory.register_type(ComplexObject2, namespace="test", typename="ComplexObject2")
    obj = ComplexObject2(f1=True, f2={-1: 2})
...
def test_register_by_id(data_file_path):
    compatible = "compatible" in data_file_path
    fory = pyfory.Fory(language=pyfory.XLANG, ref=True, compatible=compatible)
...
def test_enum_field(data_file_path):
    compatible = "compatible" in data_file_path
    fory = pyfory.Fory(language=pyfory.Language.XLANG, ref=False, compatible=compatible)
...
def test_cross_language_meta_share(data_file_path):
    ...
    fory = pyfory.Fory(language=pyfory.Language.XLANG, compatible=True, ref=True)
...
def test_field_reordering_compatibility(data_file_path):
    ...
    fory = pyfory.Fory(language=pyfory.Language.XLANG, ref=True, compatible=True)
