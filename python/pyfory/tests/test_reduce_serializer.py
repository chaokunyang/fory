...
def test_nonstrict_reduce_global():
    ...
    fory.type_resolver.get_type_info(NestedGlobalReduce)
...
def test_nonstrict_reduce_function():
    fory = Fory(xlang=False, ref=True, strict=False, compatible=False)
    fory.type_resolver.get_type_info(types.FunctionType)
    fory.type_resolver.get_type_info(FunctionCallableReduce)
    _function_factory_calls.clear()
