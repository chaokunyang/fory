    ...
    C: Codec<T>,
{
    ...
    fn read_data(context: &mut ReadContext) -> Result<Rc<T>, Error> {
        Ok(Rc::new(read_inner::<T, C>(context, false, None, None)?))
    }
    ...
    fn read_with_mode(
        ...
        read_rc::<T, C>(context, ref_mode, read_type_info, None, None)
    }
    ...
    fn read_with_type_info(
        ...
        read_rc::<T, C>(context, ref_mode, false, Some(type_info), None)
    }
...
fn read_rc<T: 'static, C: Codec<T>>(
    ...
    match ref_mode {
        RefMode::None => Ok(Rc::new(read_inner::<T, C>(
            context,
            read_type_info,
            type_info,
            remote_field_type,
        ...
        RefMode::NullOnly => {
            if context.reader.read_i8()? == RefFlag::Null as i8 {
                return RcCodec::<T, C, false, false>::default_value(context);
            }
            Ok(Rc::new(read_inner::<T, C>(
                context,
                read_type_info,
                type_info,
                remote_field_type,
        ...
        RefMode::Tracking => match context.ref_reader.read_ref_flag(&mut context.reader)? {
            RefFlag::Null => RcCodec::<T, C, false, false>::default_value(context),
            RefFlag::Ref => {
            ...
            RefFlag::NotNullValue => Ok(Rc::new(read_inner::<T, C>(
                context,
                read_type_info,
                type_info,
                remote_field_type,
            ...
            RefFlag::RefValue => {
                ...
                let value = Rc::new(read_inner::<T, C>(
                    context,
                    read_type_info,
                    type_info,
                    remote_field_type,
