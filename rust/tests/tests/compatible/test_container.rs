...
fn container_outer_auto_conv() {
    ...
    let mut read_context = ReadContext::new_from_fory(reader, &fory).unwrap();
    assert_eq!(
        Some(basic_list()),
    ...
    assert_eq!(
        basic_list(),
        fory.deserialize_with_context::<Vec<String>>(
    ...
    assert_eq!(
        basic_set(),
        fory.deserialize_with_context::<HashSet<String>>(
            &mut ReadContext::new_from_fory(Reader::new(bins.pop().unwrap().as_slice()), &fory)
                .unwrap()
        )
        .unwrap()
...
fn collection_inner() {
    ...
    for fory in [fory1, fory2] {
        ...
        assert_eq!(
            item_list(),
            fory.deserialize_with_context::<Vec<Item>>(
                &mut ReadContext::new_from_fory(Reader::new(bins.pop().unwrap().as_slice()), &fory)
                    .unwrap()
            )
            .unwrap()
        ...
        assert_eq!(
            item_set(),
            fory.deserialize_with_context::<HashSet<Item>>(
                &mut ReadContext::new_from_fory(Reader::new(bins.pop().unwrap().as_slice()), &fory)
                    .unwrap()
            )
            .unwrap()
        ...
        assert_eq!(
            ...
            fory.deserialize_with_context::<Vec<Option<String>>>(
                &mut ReadContext::new_from_fory(Reader::new(bins.pop().unwrap().as_slice()), &fory)
                    .unwrap()
            )
            .unwrap()
        ...
        assert_eq!(
            ...
            fory.deserialize_with_context::<Vec<Option<Item>>>(
                &mut ReadContext::new_from_fory(Reader::new(bins.pop().unwrap().as_slice()), &fory)
                    .unwrap()
            )
            .unwrap()
...
fn map_inner() {
    ...
    for fory in [fory1, fory2] {
        ...
        assert_eq!(
            basic_map(),
            fory.deserialize_with_context::<HashMap<String, String>>(
                &mut ReadContext::new_from_fory(Reader::new(bytes.as_slice()), &fory).unwrap()
            )
            .unwrap()
        );
        ...
        assert_eq!(
            item_map(),
            fory.deserialize_with_context::<HashMap<Item, Item>>(
                &mut ReadContext::new_from_fory(Reader::new(bytes.as_slice()), &fory).unwrap()
            )
            .unwrap()
        );
        ...
        let bytes = fory
            .serialize_with_context(&nullable_basic_map(false), &mut write_context)
            .unwrap();
        assert_eq!(
        ...
        let bytes = fory
            .serialize_with_context(&nullable_item_map(false), &mut write_context)
            .unwrap();
        assert_eq!(
...
fn complex() {
    ...
    for fory in [fory1, fory2] {
        ...
        assert_eq!(
            nested_collection(),
            fory.deserialize_with_context::<Vec<HashSet<Item>>>(
                &mut ReadContext::new_from_fory(Reader::new(bins.pop().unwrap().as_slice()), &fory)
                    .unwrap()
            )
            .unwrap()
