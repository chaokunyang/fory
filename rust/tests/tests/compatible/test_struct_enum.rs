...
fn simple_write_continuous() {
    ...
    result.iter().for_each(|x| {
        ...
        assert_eq!(animal.f3, x.f3);
        assert_eq!(animal.f5, x.f5);
        assert_eq!(animal.f6, x.f6);
        assert_eq!(animal.f7, x.f7);
        assert_eq!(animal.last, x.last);
    });
}
