...
extension (builder: ForyJsonBuilder)
  /** Registers an explicitly derived closed Scala 3 enum schema. */
  def register[T](using codec: ScalaJsonCodec[T], tag: ClassTag[T]): ForyJsonBuilder =
    builder.registerCodec(tag.runtimeClass.asInstanceOf[Class[T]], codec)
