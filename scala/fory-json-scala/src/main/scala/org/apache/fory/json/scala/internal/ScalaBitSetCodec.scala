...
private[scala] final class ScalaBitSetCodec(mutableResult: Boolean)
  ...
  override def writeString(writer: StringJsonWriter, value: scala.collection.BitSet): Unit = {
    if (value == null) {
      writer.writeNull()
      return
    }
  ...
  override def writeUtf8(writer: Utf8JsonWriter, value: scala.collection.BitSet): Unit = {
    if (value == null) {
      writer.writeNull()
      return
    }
  ...
  // Concrete readers keep bit indices on their native integer parser without generic char access.
  override def readLatin1(reader: Latin1JsonReader): scala.collection.BitSet = {
  ...
  override def readUtf16(reader: Utf16JsonReader): scala.collection.BitSet = {
    if (reader.tryReadNullToken()) return null
    reader.enterDepth()
    val inputStart = reader.position()
    reader.expectNextToken('[')
  ...
  override def readUtf8(reader: Utf8JsonReader): scala.collection.BitSet = {
    if (reader.tryReadNullToken()) return null
    reader.enterDepth()
    val inputStart = reader.position()
    reader.expectNextToken('[')
